...
# outliers from z5z5 we cannot remove because they are used for plags
data <- data %>%
  filter(!(plagTypeGroup == "z5z5" & first == "student1649" & second == "student2371") & !(plagTypeGroup == "z5z5" & first == "student2371" & second == "student1649"))


#data <- data %>%
#  filter(
#    (grepl("^gpt", first) & grepl("^gpt3-", first)) | 
#      (grepl("^gpt", second) & grepl("^gpt3-", second)) |
#      (!grepl("^gpt", first) & !grepl("^gpt", second))
#  )
...
generate_diff_table(summary_data_filtered, "gpt-obf")
...
do_the_plot <- function(data, metric, legend_position = "none") {
  ...
  plot <-
    ...
    #stat_boxplot(width = 0.5,geom ='errorbar') + 
...
width100 <- 15.24
height = 7 # 16  #width100 / 1.6 #/ 1.34

plot_and_save <- function(width, metric) {
  do_the_plot(data, metric, "bottom")
  file_name <-
    paste('./output/eval-chatgpt-obf_', metric, '.pdf', sep = "")
  ggsave(file_name,
         device = cairo_pdf,
         width = width,
...
generate_table(test_results2, "to-base-gptobf", 6, c("ds", "Variant", "Pairs", "$p$", "$W$", "$\\delta$", "$\\delta\\,Int.$", "$\\delta$ 95\\% CI", "n"))
