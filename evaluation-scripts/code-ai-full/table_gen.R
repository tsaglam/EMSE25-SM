...
generate_diff_table <- function(df, label) {
  ...
  df_filtered <- df %>%
    ...
    mutate(approach = factor(approach, levels = levels)) %>%
    select(plagTypeGroup, approach, everything()) %>%
    arrange(plagTypeGroup, approach)
  
  custom_colnames <- c("Dataset", "Approach", "Median", "Mean", "$Q_1$", "$Q_2$", "$\\Delta$ Mean", "$\\Delta$ Median", "$\\Delta$ IQR")
  ...
  print(
    latex_table,
    file = paste0("./output/table-", label, ".tex"),
...
insert_multirows <- function(data, variants_per_dataset) {
  ...
  data <- data %>%
    ...
    mutate(dataset = ifelse(
      ...
      paste0("\\multirow{", variants_per_dataset, "}{*}{", dataset, "}"),
      ""
    )) %>%
    ungroup()
  return(data)
