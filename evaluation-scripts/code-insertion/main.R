...
generate_diff_table(summary_data_filtered, "insert")
...
width100 <- 15.24 # 14 / 2
height = 21 #16 # / 1.6 #/ 1.34

plot_and_save <- function(width, metric) {
  do_the_plot(data, metric, "bottom")
  file_name <-
    paste('./output/eval-insertion_', metric, '.pdf', sep = "")
  ggsave(file_name,
         device = cairo_pdf,
         width = width,
...
generate_table(test_results2, "to-base-insert", 6, c("ds", "Variant", "Pairs", "$p$", "$W$", "$\\delta$", "$\\delta\\,Int.$", "$\\delta$ 95\\% CI", "n"))
#test_results <- stat_test_table(data)
#generate_table(test_results, "sig-plag-orig-insert", 4, c("Dataset", "Variant", "$p$", "$W$", "$\\delta$", "$\\delta\\,Int.$", "$\\delta$ 95\\% CI"))
