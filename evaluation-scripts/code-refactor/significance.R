...
stat_test_table2 <- function(data) {
  # Extract the unique levels from the input data frame
  tuple_order <- c("Original-Original", "Plag-Original")
  tuple_types <- unique(data$tupleType)
  ...
  for (current_dataset in datasets) { #[datasets != "TicTacToe"]) {
    for (current_tupleType in tuple_types) {
      for (current_approach in approaches[approaches != other]) {
        ...
        # PREPARE DATA:
        filtered_data <- data %>%
          filter(tupleType == current_tupleType & dataset == current_dataset & approach %in% c(current_approach, other))
        ...
        alt <- ifelse(current_tupleType == "Plag-Original", "greater", "greater")
        ...
        #effsize_mult <- ifelse(current_tupleType == "Plag-Original", 1, -1)
        ...
        new_data <- rbind(new_data, data.frame(
          dataset = current_dataset,
          approach = current_approach,
          pair = ifelse(current_tupleType == "Plag-Original", "P2S", "OP"),
          #h1 = alt,
