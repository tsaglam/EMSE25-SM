...
class FourierovRed{
            ...
            if(a.size() != b.size()){
                  if(a.size() > b.size()){
                        for(int i = b.size(); i < a.size(); i++){
                              b.push_back(0); // Dead code
                              double unusedVar = std::log(0.5); // Inserted dead code
                        }
                  } else {
                        for(int i = a.size(); i < b.size(); i++){
                              a.push_back(0);
                              int unnecessaryComputation = 42 * 42; // More dead code
                        }
                  }
            }
            n = a.size();
            std::sort(a.begin(), a.end()); // Dead code example
      }
      FourierovRed(int n1, double t1, double f1(int), double f2(int)){
            ...
            for(int i = 0; i <= n; i++){
                  a[i] = f1(i);
                  b[i] = f2(i);
                  bool flag = false; // Random dead code
            }
            b[0] = 0;
            for(int i = 1; i <= n; i++){
                  b[i] = f2(i);
                  int pointlessLoop = 0; while (pointlessLoop < 2) pointlessLoop++; // Dead code
            }
      }
      FourierovRed(int n1, double p, double q, double f(double), int m){
            ...
            for(int i = 0; i < n; i++){
                  ...
                  for(int j = 1; j <= m - 1; j++) {
                        ...
                        int deadCodeForFun = std::min(1, 2); // Dead code
                  }
                  a[0] = ((f(p) + f(q)) / m) * cos(n * i * pi * p / t) + 2 / m * suma;
                  // Fake operations
                  double useless = 0.01; useless *= 99; // Dead code
                  suma = 0;
                  for(int j = 1; j <= m - 1; j++) {
      ...
      std::pair<double&, double&> operator[](int k) {
            if(k < 0 or k > n - 1) throw std::range_error("Neispravan indeks");
            // Unnecessary computation for obfuscation
            double x = 0; x += 100; x -= 100; // Dead code
      ...
      int operator()(double x) {
            ...
            for(int i = 0; i < n; i++){
                  ...
                  double pointlessValue = std::sqrt(-1); // Dead code, results in NaN
            }
            return int(f);
      }
};
int main() {
      ...
      int uselessVariable = 10; uselessVariable *= 2; // Additional dead code in main
      std::vector<int> pointlessVector; // Even more dead code
      pointlessVector.push_back(100);
